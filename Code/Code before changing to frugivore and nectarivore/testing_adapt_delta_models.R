...
# Set max tree depth.
tree_depth <- c(5,6,7,8,9,10)

# Expand the grid.
all_combos <- expand.grid(model_type, center, data_type, tree_number, tree_depth)
...
tree_depth <- all_combos[array_number, 5] %>% as.numeric()
...
# Simple models.
model_pathway <- paste0("Results/Models/Latitude/", model_type, "_depth", tree_depth, "_", data_type, "_", tree_number, ".rds") 
...
brms_model <- brm(
  ...
  backend = "cmdstanr",
  threads = threading(4),
  control = list(adapt_delta = 0.6, step_size = 1, max_treedepth = tree_depth)
)
toc()
...
# Append the table.
write.table(model_threads, "testing_treedepth.csv", sep = ",", append = TRUE, 
            row.names = FALSE, col.names=!file.exists("testing_treedepth.csv"))
