...
# Phylo effect
model_formula <- "sexual_score ~ terr_bi_c + migration_bi_c + diet_c + 
diet_c*temp_seasonality_z + diet_c*terr_bi_c + (1|gr(tree_tip, cov=A))"
 
# brms formula.
brms_formula <- brmsformula(model_formula, family = cumulative(threshold = "equidistant"), decomp = "QR")

# Simple models.
model_pathway <- paste0("Results/Models/May/", data_type, "_", tree_number, ".rds") 
