...
combine_brms <- function(model_pattern){
  pattern_matched <- lat_files %>% str_subset(pattern = model_pattern)
...
# Read in latitudinal models.
all_lat_models <- combine_brms("all_centered_all")  # 49 done
high_lat_models <- combine_brms("all_centered_high")  # 50 done

# Primary consumers.
primary_all_lat_models <- combine_brms("primary_centered_all")  # 48 done 
waprimary_high_lat_models <- combine_brms("primary_centered_hig")  # 48 done 

# Secondary consumers.
secondary_all_lat_models <- combine_brms("secondary_centered_all")  # 50 done
secondary_high_lat_models <- combine_brms("secondary_centered_high") # 44 done


# Read in adapt delta test models.
lat_05_models <- combine_brms("primary_0.5_high")
lat_06_models <- combine_brms("primary_0.6_high")
lat_07_models <- combine_brms("primary_0.7_high")

depth5_models <- combine_brms("depth5")
depth6_models <- combine_brms("depth6")


plot(depth5_models)
lat_files
summary(lat_05_models)
summary(lat_06_models)
summary(lat_07_models)
