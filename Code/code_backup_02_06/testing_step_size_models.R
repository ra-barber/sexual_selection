...
# Set max tree depth.
step_size <- c(1,2,5,10,100,1000)

# Expand the grid.
all_combos <- expand.grid(model_type, center, data_type, tree_number, step_size)
...
step_size <- all_combos[array_number, 5] %>% as.numeric()
...
# Simple models.
model_pathway <- paste0("Results/Models/Latitude/", model_type, "_size", step_size, "_", data_type, "_", tree_number, ".rds") 
...
brms_model <- brm(
  ...
  backend = "cmdstanr",
  threads = threading(4),
  control = list(adapt_delta = 0.6, step_size = step_size, max_treedepth = 10)
)
toc()
...
# Append the table.
write.table(model_threads, "testing_stepsize.csv", sep = ",", append = TRUE, 
            row.names = FALSE, col.names=!file.exists("testing_stepsize.csv"))
