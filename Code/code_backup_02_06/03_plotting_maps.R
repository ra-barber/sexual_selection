...
grouped_lat_data <- phylo_data %>% filter(binned_lat != 75) %>% group_by(binned_lat)
...
frug_nect_lat_data <- phylo_data %>% filter(binned_lat != 75) %>% 
  group_by(binned_lat, diet_binary) %>% average_lat_bins()

# Group the data by herbivores or not and bins.
herb_lat_data <- phylo_data %>% filter(binned_lat != 75) %>% 
  group_by(binned_lat, herbivore_niche) %>% average_lat_bins()
...
## Other groupings of sexual selection ##
# mono_lat_data <- phylo_data %>% filter(sexual_score < 3) %>% 
#   filter(binned_lat != 75) %>% group_by(binned_lat) %>% average_lat_bins()
# sexbin_lat_data <- grouped_lat_data %>% average_lat_bins("sexual_binary")
# sextri_lat_data <- grouped_lat_data %>% average_lat_bins("sexual_tri")
...
# Source to get side plots for figures.
