...
if (center == "centered"){
  ...
  centered_responses <- c("temp_log", "migration_binary", "trophic_binary", "territory_binary")
  names(centered_responses) <- c("temp", "mig", "tro", "terr")
  model_response <- centered_responses[[response]]
}
...
# Specify model pathway for brms models.
model_pathway <- paste0("Results/Models/Univariate/", response, "/", center, "/", data_type, "/", tree_number)
model_name <- paste0(response, "_", center, "_", data_type, "_", tree_number)

check_point_folder <-  paste0("Results/Models/Univariate/Checkpoints")
dir.create(check_point_folder, recursive = TRUE)
model_pathway <- paste0(check_point_folder, "/", model_name)

# Create folder if it doesn't exist.
if (!dir.exists(model_pathway)){
    create_folder(folder_name = model_name, path  = check_point_folder)
  }
