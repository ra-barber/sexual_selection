...
all_prim_uncen_data <- readRDS(paste0(first_half, "primary_uncentered_all_data.rds"))
all_frug_uncen_data <- readRDS(paste0(first_half, "frugivore_uncentered_all_data.rds"))
all_sec_uncen_data <- readRDS(paste0(first_half, "secondary_uncentered_all_data.rds"))
all_invert_uncen_data <- readRDS(paste0(first_half, "invertivore_uncentered_all_data.rds"))
...
frug_nect_lat_data <- model_data %>% filter(binned_lat != 75) %>% 
  group_by(binned_lat, diet_binary) %>% average_lat_bins()

# Group the data by herbivores or not and bins.
herb_lat_data <- model_data %>% filter(binned_lat != 75) %>% 
