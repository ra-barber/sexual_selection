...
univariate_files <- list.files(path = c("Z:/home/sexual_selection/Results/Models/Univariate/"), 
...
all_temp_models <- combine_brms("temp_uncentered_all", file_pathways = univariate_files) 
all_mig_models <- combine_brms("mig_uncentered_all", file_pathways = univariate_files) 
all_tro_models <- combine_brms("tro_uncentered_all", file_pathways = univariate_files) 
all_terr_models <- combine_brms("terr_uncentered_all", file_pathways = univariate_files) 

# Extrat model predictions for plotting models.
all_temp_predictions <- conditional_effects(all_temp_models)[[1]]
all_mig_predictions <- conditional_effects(all_mig_models)[[1]]
all_tro_predictions <- conditional_effects(all_tro_models)[[1]]
all_terr_predictions <- conditional_effects(all_terr_models)[[1]]

# Extract r-squared as most time consuming bit.
all_temp_r_squared <- Bayes_R2_MZ(all_temp_models)
all_mig_r_squared <- Bayes_R2_MZ(all_mig_models)
all_tro_r_squared <- Bayes_R2_MZ(all_tro_models)
all_terr_r_squared <- Bayes_R2_MZ(all_terr_models)

# All details.
all_temp_deets <- list(all_temp_models, all_temp_predictions, all_temp_r_squared)
all_mig_deets <- list(all_mig_models, all_mig_predictions, all_mig_r_squared)
all_tro_deets <- list(all_tro_models, all_tro_predictions, all_tro_r_squared)
all_terr_deets <- list(all_terr_models, all_terr_predictions, all_terr_r_squared)

# Export model details for future use.
saveRDS(all_temp_deets, file = "Results/Models/Univariate/temp_model_data.rds")
saveRDS(all_mig_deets, file = "Results/Models/Univariate/mig_model_data.rds")
saveRDS(all_tro_deets, file = "Results/Models/Univariate/tro_model_data.rds")
saveRDS(all_terr_deets, file = "Results/Models/Univariate/terr_model_data.rds")
...
# # P values for side plots.
# temp_p_value <- as.numeric(p_map(all_temp_models, parameters = "temp_log")[2])
# mig_p_value <- as.numeric(p_map(all_mig_models, parameters = "binary")[2])
# tro_p_value <- as.numeric(p_map(all_tro_models, parameters = "binary")[2])
# terr_p_value <- as.numeric(p_map(all_terr_models, parameters = "binary")[2])
...
# temp_cor_label <- paste0("\U03B2 = ", temp_estimate, "\np < 0.001")
# mig_cor_label <- paste0("\U03B2 = ", mig_estimate, "\np < 0.001")
# tro_cor_label <- paste0("\U03B2 = ", tro_estimate, "\np = ", as.character(format(round(tro_p_value, 2), nsmall = 2)))
# terr_cor_label <- paste0("\U03B2 = ", terr_estimate, "\np < 0.001")
