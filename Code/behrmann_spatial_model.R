...
xmin = -18000000
xmax = 18000000
#ymin = -8000000 # Could set as 60 degrees.
...
blank_lowres_behr <- raster(ncols=72, nrows=28, xmn = xmin, xmx = xmax, ymn = ymin, ymx = ymax, crs = behr_proj)
blank_medres_behr <- raster(ncols=360, nrows=140, xmn = xmin, xmx = xmax, ymn = ymin, ymx = ymax, crs = behr_proj)
...
land <- fasterize(land, blank_200_behr)

raster_template <- blank_lowres_behr
raster_template <- blank_medres_behr
raster_template <- blank_200_behr
...
# Remove big objects.
#rm(sex_ranges)
save(list = ls(pattern =  "_raster"), file = "Results/SAR_200_rasters.Rdata")
...
save(list = ls(pattern =  "_sf"), file = "Results/SAR_200_sfdata.Rdata")
...
# high_fruit_sex_model <- scaled_run_spatial_model(high_fruit_sex_sf) #62 seconds. (#92 seconds 2nd time)
