...
xmin = -18000000
xmax = 18000000
#ymin = -8000000 # 80 degrees.
ymin = -6000000 #  60 degrees.
#ymin = -7000000 
ymax = 8000000

options(scipen = 999)

# # Define 500km and 100 km square grids (At 80 degrees minus lat.)
# blank_lowres_behr <- raster(ncols=72, nrows=32, xmn = xmin, xmx = xmax, ymn = ymin, ymx = ymax, crs = behr_proj)
# blank_medres_behr <- raster(ncols=360, nrows=160, xmn = xmin, xmx = xmax, ymn = ymin, ymx = ymax, crs = behr_proj)

# At 70 degrees lat.
#blank_500_behr <- raster(ncols=72, nrows=30, xmn = xmin, xmx = xmax, ymn = ymin, ymx = ymax, crs = behr_proj)
#blank_200_behr <-raster(ncols=180, nrows=75, xmn = xmin, xmx = xmax, ymn = ymin, ymx = ymax, crs = behr_proj)
#blank_100_behr <- raster(ncols=360, nrows=150, xmn = xmin, xmx = xmax, ymn = ymin, ymx = ymax, crs = behr_proj)

# Define blank rasters at 60 degrees lat.
...
# Set template.
raster_template <- blank_500_behr
...
# Remove big objects.
#rm(sex_ranges)
save(list = ls(pattern =  "_raster"), file = "Results/SAR_500_rasters.Rdata")
...
save(list = ls(pattern =  "_sf"), file = "Results/SAR_500_sfdata.Rdata")
...
# Figure 2.
sex_model <- run_spatial_model(sex_sf)   # 105 seconds.
cert_model <- run_spatial_model(cert_sf)

# Figure 3.
p_sex_model <- run_spatial_model(p_sex_sf) #200km = 61 seconds
s_sex_model <- run_spatial_model(s_sex_sf)
fruit_sex_model <- run_spatial_model(fruit_sex_sf) #62 seconds. (#92 seconds 2nd time)
invert_sex_model <- run_spatial_model(invert_sex_sf)

# Extended data figure 3.
h_sex_model <- run_spatial_model(h_sex_sf) # 200km took = 99 seconds.
m_sex_model <- run_spatial_model(m_sex_sf) # 117 seconds.

# Extended data figure 4.
mig_sex_model <- run_spatial_model(mig_sex_sf) # 56 seconds.
no_mig_sex_model <- run_spatial_model(no_mig_sex_sf)

# Extended data figure 5.
terr_sex_model <- run_spatial_model(terr_sex_sf)
no_terr_sex_model <- run_spatial_model(no_terr_sex_sf)  # 200 km took 97 seconds.

# Extended data figure 8.
p_terr_model <- run_spatial_model(p_terr_sf)
s_terr_model <- run_spatial_model(s_terr_sf) # 200 = 113 seconds.
...
scaled_sex_model <- scaled_run_spatial_model(sex_sf)   # 105 seconds.
scaled_cert_model <- scaled_run_spatial_model(cert_sf)

scaled_p_sex_model <- scaled_run_spatial_model(p_sex_sf) #200km = 61 seconds
scaled_s_sex_model <- scaled_run_spatial_model(s_sex_sf)
scaled_fruit_sex_model <- scaled_run_spatial_model(fruit_sex_sf) #62 seconds. (#92 seconds 2nd time)
scaled_invert_sex_model <- scaled_run_spatial_model(invert_sex_sf)

scaled_h_sex_model <- scaled_run_spatial_model(h_sex_sf) # 200km took = 99 seconds.
scaled_m_sex_model <- scaled_run_spatial_model(m_sex_sf) # 117 seconds.

scaled_mig_sex_model <- scaled_run_spatial_model(mig_sex_sf) # 56 seconds.
scaled_no_mig_sex_model <- scaled_run_spatial_model(no_mig_sex_sf)
scaled_terr_sex_model <- scaled_run_spatial_model(terr_sex_sf)
scaled_no_terr_sex_model <- scaled_run_spatial_model(no_terr_sex_sf)  # 200 km took 97 seconds.

scaled_p_terr_model <- scaled_run_spatial_model(p_terr_sf)
scaled_s_terr_model <- scaled_run_spatial_model(s_terr_sf) # 200 = 113 seconds.
...
summary(high_sex_model)
summary(high_p_sex_model)
summary(high_s_sex_model)
summary(high_fruit_sex_model)
summary(high_invert_sex_model)

summary(high_mig_sex_model)
summary(high_no_mig_sex_model)
summary(high_terr_sex_model)
summary(high_no_terr_sex_model)
...
save(list = ls(pattern =  "_model"), file = "Results/SAR_500_models.Rdata")
load("Results/SAR_500_models.Rdata")
...
sar_lat_side_plot <- function(data_set, ylabel = "", ylimits = c(0,1.2), ybreaks = c(0,0.5,1), 
  ...
  ggplot(data_set, aes(x = abs_lat, y = layer)) +
    geom_point(aes(colour = binned_layer),#position = position_jitter(width = 1), 
               size = 3, alpha = 0.9) + 
...
# Export the plots.
save(list = ls(pattern =  "lat_plot"), file = "Plots/Maps/normal_behr_500_latitudinal_sideplots.Rdata")
gc()
