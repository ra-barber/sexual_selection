...
model_formula <- "sexual_score ~ (terr_bi_c + migration_bi_c + trophic_level_c + temp_seasonality_z)^2 + (1|gr(tree_tip, cov=A))"

# brms formula.
brms_formula <- brmsformula(model_formula, family = cumulative(), decomp = "QR")

# Simple models.
model_pathway <- paste0("Results/Models/Interaction_longer/", data_type, "_", tree_number, ".rds") 
