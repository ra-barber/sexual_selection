...
# Read in raster.
raster_pathways <- list.files("../Data/Chelsa/", full.names = TRUE)
env_raster <- rast(raster_pathways[array_number])
...
envar_be <- project(env_raster, behr)

# Project to new coordinates.
#envar_be <- projectRaster(env_raster, crs = behr)
...
gdalwarp(srcfile = raster_pathways[array_number], "test.tif", t_srs = behr)



envar <- stack(lapply(list.files("../Data/Chelsa/", full.names = TRUE),raster))
#change order()
...
#               envar[[2]], # Annual precipitation
#               envar[[3]], # Precip wettest month.
#               envar[[4]], # Precip driest month.
#               envar[[5]], # Precip seasonality
...
envar_be <- readRDS("../Data/behr_raster_layers.rds")
...
raster_i <- fasterize(st_as_sf(sex_ranges$Shape[1]), r) #fasterize polygon onto raster
...
results <- data.frame(species = split$value$bird_tree_name[1],
                      bio1,
                      bio2,
                      bio4,
                      bio5,
