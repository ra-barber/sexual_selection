...
names(clade_colours) <- c(#"", 
                          ...
                          "Columbimorphae", "Otidimorphae", "Gruiformes",
                           "Mirandornithes", "Charadriifromes", "Eurpygimorphae",
...
clade_plot <- family_plot +
  ...
  tree_image(83, "Data/phylopic/menura.png", i_size*2, inner-4, 2) +
...
ggsave("Plots/Trees/prum_highlighted_clades.png", dpi = 900, width = 10, height = 10)
...
clade_plot + 
  theme(text = element_text(face = "bold"), 
        ...
        legend.margin = margin(-9, 6, 6, 6),
        plot.margin = margin(l = -40, b = -75, t = -35, r = -40))# +
  #annotate("text", x = 2, y =-2, label = "Clade")


ggsave("Plots/Trees/test_clades.png", dpi = 900, width = 10, height = 10)





figure_tree <- clade_plot + 
  # Makes the tree the right size/zoom.
  xlim(0,92) +
  theme(text = element_text(face = "bold"), 
        #legend.title=element_text(vjust = -5.5, hjust = 0.11), 
        ...
        #legend.margin = margin(-9, 6, 6, 6),
        plot.margin = margin(l = -40, b = -175, t = -175, r = -40))
...
###############################################################################
              #### Create family average side plots ####
...
# Territory plot.
terr_plot <- teritory_data %>% ggplot(aes(x=territory_binary, y=sqrt(sexual_mean))) + 
  plot_settings + scale_x_discrete(labels = c("Low", "High")) +
  xlab("Territoriality")
...
mig_plot <- migration_data %>% ggplot(aes(x=migration_binary, y=sqrt(sexual_mean))) + 
  plot_settings + scale_x_discrete(limits = c("Weak", "Strong"), labels = c("Short", "Long")) +
  xlab("Migration distance")

# Group together side plots.
...
hist(model_data$chick_pc1, breaks = 100)
