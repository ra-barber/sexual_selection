...
# Model
model_type <- c("equidistant", "normal")
...
# Expand the grid.
all_combos <- expand.grid(model_type, center, data_type, tree_number)
...
# Phylo effect
model_formula <- "sexual_score ~ terr_bi_c + migration_bi_c + trophic_level_c + chick_z + 
trophic_level_c*temp_seasonality_z + (1|gr(tree_tip, cov=A))"
 
# brms formula.
if (model_type == "equidistant"){
...
# Simple models.
model_pathway <- paste0("Results/Models/Simple/",  model_type, "_", center, "_", data_type, "_", tree_number, ".rds") 
