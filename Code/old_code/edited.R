...
# model_formula <- "sexual_score ~ gen_z + devo_binary + migration_binary + territory_binary +
#                   trophic_binary*temp_seasonality_z +
#                   trophic_binary*npp_z + trophic_binary*territory_binary +
#                   devo_binary*temp_seasonality_z +
#                   devo_binary*npp_z + devo_binary*territory_binary +
#                   (1|gr(tree_tip, cov=A))"

model_formula <- "sexual_score ~ gen_z + wang_edited + migration_binary + territory_binary +
                  trophic_binary*temp_seasonality_z +
                  trophic_binary*npp_z + trophic_binary*territory_binary +
                  (1|gr(tree_tip, cov=A))"
...
# Changing ref group.
#model_pathway <- paste0("Results/Models/newref_devo_", data_type, "_", tree_number, ".rds")  # Changing the reference to altrical (larger group)            ~~~ DONE ~~~~
#model_pathway <- paste0("Results/Models/inter_wang_", data_type, "_", tree_number, ".rds") # Using normal wang with interactions.                         ~~~ DONE ~~~~

# Testing other quick  devo methods.
#model_pathway <- paste0("Results/Models/fed_", data_type, "_", tree_number, ".rds") # Using fed_sqrt instead.                                ~~~~ DONE ~~~~
#model_pathway <- paste0("Results/Models/greisser_", data_type, "_", tree_number, ".rds") # Using gressier devo models.                           ~~~~ DONE ~~~~
#model_pathway <- paste0("Results/Models/chick_", data_type, "_", tree_number, ".rds") # # Add chick devlopment.                                 ~~~~ DONE ~~~~


# Adding quick interactions with development.
# model_pathway <- paste0("Results/Models/inter_fed_", data_type, "_", tree_number, ".rds") # Adding fed with interactions.                             ~~~~ DONE ~~~~
# model_pathway <- paste0("Results/Models/inter_greisser_", data_type, "_", tree_number, ".rds") # Adding greiseer with interactions                       ~~~~ DONE ~~~~
#model_pathway <- paste0("Results/Models/inter_chick_", data_type, "_", tree_number, ".rds")  # Adding chick pc1 with interactions.                         ~~~~ DONE ~~~~

# Adding in the slower other devos.
model_pathway <- paste0("Results/Models/edited_", data_type, "_", tree_number, ".rds") # Using wang_edited
# model_pathway <- paste0("Results/Models/inter_edited_", data_type, "_", tree_number, ".rds") # Using wang edited with interactions

# Removing gen length.
#model_pathway <- paste0("Results/Models/nogen_devo_", data_type, "_", tree_number, ".rds") # Removing generation length                         ~~~~ DONE ~~~~
#model_pathway <- paste0("Results/Models/nogen_", data_type, "_", tree_number, ".rds") # Removing generation length and devo                             ~~~~ DONE ~~~~

# Centered
#model_pathway <- paste0("Results/Models/cen_devo_", data_type, "_", tree_number, ".rds")  # Changing the reference to altrical (larger group) and centering.
