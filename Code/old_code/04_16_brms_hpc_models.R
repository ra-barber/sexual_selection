...
# Data type.
model_type <- all_combos[array_number, 2] %>% as.character()

# Data type.
center <- all_combos[array_number, 3] %>% as.character()
...
# Phylo effect
phy_effect <- " + (1|gr(tree_tip, cov=A))"

# Chick and npp interaction (both already centered).
chick_npp_inter <- " + chick_z*npp_z"

# Change formula for centered and uncentered.
if(center == "uncentered"){
  # Basic model formula.
  basic <- "sexual_score ~ territory_binary + migration_binary + trophic_binary + chick_z + trophic_binary*temp_seasonality_z + trophic_binary*territory_binary"
  # Npp interaction.
  npp_inter <- " + trophic_binary*npp_z"
  # Chick interaction.
  chick_inter <- " + chick_z*temp_seasonality_z + chick_z*territory_binary"
} else {
  # Basic model formula.
  basic <- "sexual_score ~ terr_bi_c + migration_bi_c + trophic_level_c + chick_z + trophic_level_c*temp_seasonality_z + trophic_level_c*terr_bi_c"
  # Npp interaction.
  npp_inter <- " + trophic_level_c*npp_z"
  # Chick interaction.
  chick_inter <- " + chick_z*temp_seasonality_z + chick_z*terr_bi_c"
}

# Change formula based on what's needed.
if (model_type == "npp"){
  # With npp interaction.
  model_formula <- paste0(basic, npp_inter, chick_inter, chick_npp_inter, phy_effect)
}
if (model_type == "no_npp"){
  # With npp interaction.
  model_formula <- paste0(basic, chick_inter, phy_effect)
}
if (model_type == "no_inter"){
  # No devo interaction.
  model_formula <- paste0(basic, npp_inter, phy_effect)
}
...
model_pathway <- paste0("Results/Models/Feb/",  model_type, "_", center, "_", data_type, "_", tree_number, ".rds") 
